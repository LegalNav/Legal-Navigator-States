	...
	<div class="container">
		<?php if (have_posts()) {
			while (have_posts()) {
                ...
                $user_state_term = get_term_by('id', $user_state_id, 'states');

                // LS Resource Fields
                $content_uuid = get_field('cr_uuid');
                $kr_uuid = get_field('kr_uuid');
                $ls_api_domain = get_field('legal_server_resource_api_domain', $user_state_term);
				...
				$supported_state_ids = array_map(
                    function ($x) {return $x->term_id;},
                    get_the_terms(get_the_ID(), 'states')
                );

				// Invalid state flag
				$not_for_current_state = !in_array(
                    $user_state_id,
                    $supported_state_ids
                );

				if ($not_for_current_state) { ?>
					<h5 class="error-text">Sorry, this resource is not available for the selected location. Please try selecting a different location.</h5>
				<?php } else { ?>
