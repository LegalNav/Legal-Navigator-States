...
function get_related_topics($data)
{
    ...
    // Only grab parent topics associated with current state
    $parent_terms = array();
    if(!empty($term_objs)){
        foreach($term_objs as $term) {
            $associated_states = get_field('associated_states', $term);
            if(is_array($associated_states)) {
                if(in_array($user_state_id, $associated_states)) {
                    ...
                    if($term->parent == 0) {
                        ...
                        $children_for_state_count = array_filter($children, function($x) {
                            $childs_associated_states = get_field('associated_states', get_term($x));

                            if(is_array($childs_associated_states)) {
                                return in_array($_SESSION['user_state_id'], $childs_associated_states);
                            }
                        }); 
                        
                        if($children_for_state_count) {
