...
function render_topic_results()
{
	...
	if (empty($state)) {
		echo 'Please select a state to view this topic.';
		wp_die();
	}
	if (empty($topic)) {
			...
			foreach ($resource_array as $resource_item) {
				...
				if (in_array($taxonomy, $terms)) {
					if ((!$section_opened) && (!empty($post_icon_id))) {
						...
						<?php if ((!empty($resource)) && ($resource == 'organization') && (get_field('show_find_location_filter', get_term_by('id', $state, 'states')))) { ?>
