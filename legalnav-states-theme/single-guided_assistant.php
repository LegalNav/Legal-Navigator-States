 ...
 */
    ...
    // Get current stata court logo url
    $court_logo_url = get_field('court_logo', $user_state_term);
    ...
    <?php
        if (have_posts()) {
            ...
            while (have_posts()) {
                ...
                // Get GA's state ids
                function get_term_id($x) {
                    return $x->term_id;
                }
                $supported_state_ids = array_map('get_term_id', get_the_terms(get_the_ID(), 'states'));

                // Check if user state is in GAs state
                $user_state_supported = in_array($user_state_id, $supported_state_ids);

                if(!($user_state_id && $user_state_supported)) { ?>
                    <h5 class="error-text">Sorry, no Guided Assistants were found. Please try selecting a different location.</h5>
                <?php } else { ?>
                    ...
                    <?php if ($guided_assistant_type == 'a2j') { ?>
                        ...
                        <div class="personal-plan-header hidden">
                            ...
                            <div class="plan-header-content">
                                ...
                                <p>The information and resources below are from Legal Navigator’s partner organizations in <?php echo $user_state_name ?>.</p>
                        ...
                        <div class="curated-experience col-md-8 col-md-offset-2 col-sm-10 col-sm-offset-1 question <?=$text_justification?>">
                            ...
                            <!-- <button class="next-btn" type=text>Continue</button> -->
                        </div>
        ...
        if ($show_court_lookup) { get_template_part("/template-parts/blocks/court_case_lookup"); }
