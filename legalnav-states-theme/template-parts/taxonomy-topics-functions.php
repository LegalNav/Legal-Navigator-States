...
function render_sidebar($user_state_id, $result_resource_items)
{
        ...
        <?php if (!empty($sidebar_heading)) { ?>
            <div class="sidebar-callout">
                ...
                if (!empty($sidebar_heading)) { ?>
                    <h3><?php echo $sidebar_heading; ?></h3>
                <?php
                }
                if (!empty($sidebar_image)) { ?>
                    <div class="topic-icon" style="background-image: url('<?php echo $sidebar_image; ?>')"></div>
                <?php
                }
                if (!empty($sidebar_text)) { ?>
                    <h6><?php echo $sidebar_text; ?></h6>
                <?php
                }
                if (!empty($sidebar_button_text)) { ?>
                    <a class="btn btn-primary" href="<?php echo $sidebar_link; ?>" target="_blank"><?php echo $sidebar_button_text; ?></a>
                <?php
                }
                ?>
...
function render_topics_branch($topic, $sub_topics, $user_state_id)
{ ?>
    ...
    if (!empty($sub_topics)) {
        ...
        foreach ($sub_topics as $sub_topic) {
            //If the state ID matches, use it, otherwise continue loop.
            if (in_array($user_state_id, get_field('associated_states', $sub_topic))) {
    ...
    render_sidebar($user_state_id, $result_resource_items);
}
...
function render_topics_leaf($topic, $user_state_id) { ?>
<div class="topics-page-box">
    ...
    <div id="topics-results" data-ajaxurl="<?=admin_url('admin-ajax.php');?>" data-page="" data-state="<?=$user_state_id?>" data-topic="<?=$topic->slug;?>" data-resource="" data-region="" data-location=""></div>
