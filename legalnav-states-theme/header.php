...
$state_has_search = get_field('show_search', $user_state_term);
?>
    ...
    <body <?php body_class(); ?>>
        <header>
			...
			<div class="header-top-left-bar">
				...
				<?php echo $user_state_id ? '<a href="javascript:;" class="user-state">'.get_term_by('term_id', $user_state_id, 'states')->name.'</a><a class="update-location-link" href="javascript:;">Update Location</a>' : '<a class="update-location-link" href="javascript:;">Select Location</a>' ?>
			</div>
			<?php
			}
			...
			<div class="header-bottom-bar">
				...
				<?php if($state_has_search) { ?>
					...
					<form id="searchform" class="searchform">
						<input type="text" name="search" class="" value="<?=$input_value;?>" data-state-id="<?=$user_state_id;?>">
			...
			<?php
				$sliced_state_feat_topics = array_slice(get_field('homepage_feat_topics', $user_state_term), 0, 3);
			?>
			<div class="search-results hidden">
				...
				<div class="state-topics related-topics">
					<?php if(count($sliced_state_feat_topics) > 0) { ?>
						...
						<div class="row">
							<?php foreach ($sliced_state_feat_topics as $topic) { ?>
		...
		<div class="location-modal hidden">
			<div class="location-modal-container">
				<a href="javascript:;" class="location-modal-close-icon" aria-label="close location selection box">
					<svg xmlns="http://www.w3.org/2000/svg" width="24.765" height="24.765" viewBox="0 0 24.765 24.765"><g transform="translate(-1766.617 -197.617)"><g transform="translate(866.977 -3067.005)"><line x2="22.154" y2="22.154" transform="translate(900.946 3265.928)" fill="none" stroke="#333" stroke-linecap="round" stroke-miterlimit="10" stroke-width="1.846"/><line x1="22.154" y2="22.154" transform="translate(900.946 3265.928)" fill="none" stroke="#333" stroke-linecap="round" stroke-miterlimit="10" stroke-width="1.846"/></g><path d="M1779,210l11.077,11.077Zm0,0-11.077,11.077Zm0,0,11.077-11.076ZM1779,210Zm-11.077-11.076L1779,210Z" fill="#0d3fb8"/></g></svg>
				</a>
				<h3>Location</h3>
				<p>Select state to see relevant information for that location.</p>
				<form id="select-state" action="/" method="post">
					<input type="hidden" id="currentpageurl" name="currentpageurl" value="<?=$input_value;?>" />
					<select id="state-selected" name="location_select" class="location-select">
					...
					<button id="state-submit" class="btn btn-primary location-btn" type="submit" name="location_update">Select</button>
				</form>
